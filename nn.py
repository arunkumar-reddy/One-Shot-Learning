...
def nonlinear(weights,nonlinearity,leak=0.2):
	if nonlinearity == 'relu':
		...
		return tf.maximum(weights,weights*leak);
...
'''Apply a deconvolution layer'''
def deconvolution(feats,output_shape,filter_h,filter_w,stride_h,stride_w,name,init_w='he',init_b=0,stddev=0.01,padding='SAME',group_id=0):
	input_depth = getshape(feats)[-1];
	output_depth = output_shape[-1];
	deconvolve = lambda feats,weights,output_shape: tf.nn.conv2d_transpose(feats,weights,output_shape,strides=[1,stride_h,stride_w,1],padding=padding);
	with tf.variable_scope(name) as scope:
		weights = weight('weights',[filter_h,filter_w,output_depth,input_depth],init=init_w,stddev=stddev,group_id=group_id);
		output = deconvolve(feats,weights,output_shape);
		biases = bias('biases',output_depth,init_b);
		output = tf.nn.bias_add(output,biases);
	return output;
...
def max_pool(feats,filter_h,filter_w,stride_h,stride_w,padding='SAME'):
	return tf.nn.max_pool(feats, ksize=[1,filter_h,filter_w,1], strides=[1,stride_h,stride_w,1],padding=padding);

'''Apply a average pooling layer'''
def avg_pool(feats,filter_h,filter_w,stride_h,stride_w,padding='SAME'):
	return tf.nn.avg_pool(feats, ksize=[1,filter_h,filter_w,1], strides=[1,stride_h,stride_w,1],padding=padding);
